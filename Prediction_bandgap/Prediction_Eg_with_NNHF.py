...
file_name_input_test_x = './Data/'+ str(filenum) +'_test_Eg.dat'
...
with tf.Session() as sess:
    ...
    for i in range(len(test_x_list)):
        test_x_data, p_Eg, AB_material_vactor = CNNs.data_label_test(test_x_list[i:(i+1)], 1)
        test_p_Eg = np.reshape(p_Eg, [-1, 1])
        test_AB_material_vector = np.reshape(AB_material_vactor, [-1, 60,1,1])
        tst_MAE, tst_MSE, tst_y_hat, tst_y_real = model.test_step(test_x_data, test_AB_material_vector, test_p_Eg)
